    ...
    linear_layer_indices = sorted(
        {
            ...
            if key.startswith("model.") and key.endswith(".weight") and state_dict[key].ndim == 2
        }
    )
    hidden_layer_sizes = [int(state_dict[f"model.{index}.weight"].shape[0]) for index in linear_layer_indices[:-1]]
    return HyperparameterizedANN(num_features, hidden_layer_sizes=hidden_layer_sizes)
...
def format_model_result(model, tensor):
    predicted_class, confidence, top_3_predictions, inference_time_ms = run_inference(model, tensor)
    top_3_text = "\n".join(f"{rank}. {prediction}" for rank, prediction in enumerate(top_3_predictions, start=1))
    return predicted_class, f"{confidence:.2f}%", top_3_text, f"{inference_time_ms:.2f} ms"
...
def format_prediction_result(sample_index, current_samples):
    ...
    results = [sample["image"], true_class]
...
def build_gallery_items(current_samples):
    return [(sample["image"], sample["gallery_caption"]) for sample in current_samples]
